...
|-------|------|---------------|------|
| 0 (current) | Spec + public scaffold + 7-surface rehearsal + fail-closed `loom action execute` + governed local worker dispatch on allow-path + kernel-CLI-backed runtime audit artifacts + parity stream with per-action OpenClaw probe artifacts | 0/7 | — |
