...
| `meridian_loom` | hosted | Planned (0/7, spec + public experimental scaffold with 7-surface rehearsal, fail-closed `loom action execute`, governed local worker dispatch on allow-path, `action enqueue` + `supervisor run` queue rehearsal, kernel-owned runtime audit artifacts, and a parity stream with per-action OpenClaw probe artifacts — [LOOM_SPEC](docs/LOOM_SPEC.md)) |
